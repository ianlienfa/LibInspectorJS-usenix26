...
function rapid_mix_fast(a, b) {
    const m = a * b;
    return BigInt.asUintN(64, m) ^ (m >> 64n);
}
function rapidhash_epilogue_fast(a, b, secret, len) {
    const m0 = a * b;
    const m1 = (BigInt.asUintN(64, m0) ^ secret[0] ^ len) * ((m0 >> 64n) ^ secret[1]);
    return BigInt.asUintN(64, m1) ^ (m1 >> 64n);
}
function rapid_mix_protected(a, b) {
    const m = a * b;
    return BigInt.asUintN(64, m) ^ (m >> 64n) ^ a ^ b;
}
function rapidhash_epilogue_protected(a0, b0, secret, len) {
    const m0 = a0 * b0;
...
function rapid_read32x2(buf, offset1, offset2) {
...
function rapid_read64(buf, offset) {
...
function rapid_readSmall(buf, offset, k) {
...
function rapidhash_internal(key, seed, secret, rapid_mix, rapidhash_epilogue) {
...
const defaultRapidMumBehaviour = 'fast';
 ...
 */
function rapidhash(message, options) {
 ...
 * @param options.seed {bigint} 64-bit unsigned seed value.
 */
function rapidhash_fast(message, options) {
 ...
 * @param options.seed {bigint} 64-bit unsigned seed value.
 */
function rapidhash_protected(message, options) {
    const { seed } = validateOptions(options);
    return rapidhash_internal(toDataView(message), seed, rapid_secret, rapid_mix_protected, rapidhash_epilogue_protected);
}

exports.rapidhash = rapidhash;
exports.rapidhash_fast = rapidhash_fast;
exports.rapidhash_protected = rapidhash_protected;
