...
def getValueOfWithLocationChain(tx, varname, rootContextNode, recurse_counter=0, PDG_on_variable_declarations_only=False, context_scope=''):
	...
	for item in results: 
		...
		for iteratorNode in currentNodes:
			if iteratorNode['Type'] == 'BlockStatement': 
				logger.info(f"[debug] block statement for node: {iteratorNode}")
				...
				# special handling for function calls, skip for now
			...
			if contextNode['Id'] == constantsModule.PROGRAM_NODE_INDEX: 
				logger.info(f"[debug] hitting root node? {contextNode}")
				continue
			...
			new_varnames = _get_unique_list(list(idents))
			logger.info(f"new_varnames: {new_varnames}")
	...
	logger.info(f"out_values: {out_values}")
	return out_values
...
def run_traversals(tx, navigation_url, webpage_directory, folder_name_of_url='xxx', document_vars=[]):
	...
	if MAIN_QUERY_ACTIVE:
		...
		for call_expr in r1:
			...
			wrapper_node_top_expression = getChildsOf(tx, t) # returns all the child of a specific node
	...
	logger.info(f"hash_or_amp_params: {hash_or_amp_params}")
	hashSymbol = "#"
	...
	# out here is only used when there's hashSymbol found, returns an array	
	return out
