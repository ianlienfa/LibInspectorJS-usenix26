class CreditCalculator:
    ...
    def __CalculateCredit(self, node, d):
        if d == 0 or d == 1:
            ...
            current_depth_credit = (2 ** (self.D - d) / (2 ** (self.D - 1) - 1)) * self.total_credit
            credit = ((node['subtree_size'] - self.depth_subtreesize_index[1]) / self.depth_subtreesize_index[d]) * current_depth_credit
            # make depth 0 and 1 credit 0, remove the node count for depth 0 and 1
