...
def generatePT(file_index, route):
    driver.get(f"http://127.0.0.1:{APP_PORT}/{route}/{file_index}")
...
def updateOne(libname, file_index, limit_globalV=[], omit_done=False):
...
def updateLibrary(libname, start_id = 0):
    ...
    for file_index in file_list:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            try:
                future = executor.submit(updateOne, libname, file_index)
...
def updateAll(st = 0):
    ...
    range_idx = [0, 1000, 1500, 2000, 2500, 3000, 3500, 4000, len(libfiles_list)]    
    for i in range(range_idx[st], range_idx[st+1]):
        fname = libfiles_list[i]
        libname = fname[:-5]
        if libname == LAST_LIB_NAME:
            LAST_LIB_VISITED = True
            continue            
        if LAST_LIB_VISITED:
            logger.info(f"updateing {fname} - {i}/{len(libfiles_list)}")
