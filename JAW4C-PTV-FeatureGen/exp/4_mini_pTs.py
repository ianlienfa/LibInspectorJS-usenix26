...
def minify_pTs(libname):
    ...
    for entry in res:
        T = tree.LabeledTree(None, entry[1])   
    ...
    #     print("tree version after load: ", G.trees[i].name)
    # for i in range(len(G.trees)):
    #     print("version:", G.trees[i].name, "original tree: ", G.trees[i])

    # Minification
    # t0_5 = G.get_equivalence()  

    ##### 
    # Level override: support for detecting library renaming
    t0 = G.get_level_override()    
    t1 = G.get_equivalence()    
    print("get_trees_metas called")
    logger.info('Get override finished.')
    ######
    ...
    G.get_mtrees_metas()
    print('self.trees[k].S', [(i, G.trees[i].S) for i in range(len(G.trees))])
    ...
    for i in range(len(G.trees)):
        ...
        mTree = G.mtrees[i]
        # print("version:", G.mtrees[i].name, "tree: ", mTree)
...
def minifyAll():
    ...
    # last_lib_name = "handsontable" # jump
    last_lib_name = "hasplayer.js"
