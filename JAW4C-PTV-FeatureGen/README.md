...
9. verification step: `/verification/utilities/analyze-parallel.js` to generate verification stats, those file will be saved in `logs/analysis`, then run `/verification/utilities/process_results.js` to generate error.json, move this file to the Bundle-PTV data directory, this information reflects the correctness of the detection results compared with the ground truth so that the user knows whether to trust the detection result or not
...
#### Verification Step
Run `node utilities/analyze-parallel.js` in *verification* directory, which creates *verification/error.json* that provides ground truth information for PTV and can be applied to PTV under the data directory for extra information support.
This is a temporary setup before doing further modification on PTV itself and for convenience on testing
...
```
for port in {6548..6568}; do
    screen -dmS "server-$port" python3 app.py -p $port
  done
```


# Aug 15 update
check freq_failed_libs.log and 5_freq_pTs.1.log to see if any library has not yet been updated
```
for port in {6548..6567}; do
...
```
node --inspect-brk utilities/analyze-p-debug.js 
```