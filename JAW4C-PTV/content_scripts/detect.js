(() => {
    ...
    class PropertyRecord {
        ...
        grow(attribute_name) {  // PR is an instance of PropertyRecord            
            ...
            // console.log("this.ptr", this.ptr)
            // I want undefined can also return true if hasownprop = true
            try{               
            ...
            try{               
                ...
                // Blocked a frame with origin "https://ieeexplore.ieee.org" 
                // from accessing a cross-origin frame.
                return false
            }
            if (typeof(old_ptr?.['hasOwnProperty']) === 'function' && old_ptr.hasOwnProperty(attribute_name)) {
                return true
            }
            else {
                return false
            }
            // console.log(this.path_list, `${attribute_name} exists!`)            
            // return true
        }

        // grow the attributes under a propertyRecord to represent one level deeper search
    ...
    class Library {
        ...
        compareWithPTree_cutoff(root) {
            ...
            if(!this.detectLocationRecord){
                console.warn("current library having invalid detectLocationRecord")
            }
            console.log("root: ", JSON.stringify(root))
            console.log("this.detectLocationRecord: ", this.detectLocationRecord) // Probably has circular dependencies, don't do JSON.stringify


            while (q.length) {
                let vertex = q.shift()
                let pr = q2.shift() // the detection location
                console.log("vertex: ", vertex)
                console.log("pr: ", pr) // Probably has circular dependencies, don't do JSON.stringify
    
                if (!this.checkComply(pr.ptr, vertex['d'])) {
                    console.log(`compareWithPTree_cutoff: [No match] no match on ${pr.ptr} and ${vertex['d']}`)
                    return false
                } 
                else {
                    console.log(`compareWithPTree_cutoff: match on ${pr.ptr} and ${vertex['d']}`)
                }

                console.log("vertex['c']: ", vertex['c'])
                for (let child of vertex['c']) {                    
                        let new_pr = new PropertyRecord()
                        let success = new_pr.growFrom(pr, child['n'])
                        console.log("child['n']: ", child['n']);   
                        if ((!success) || (typeof new_pr.ptr === "undefined")) {
                            console.log("compareWithPTree_cutoff: [No match]: growFrom() not success!")
                            return false
                        } else {
                            q.push(child)
                            q2.push(new_pr)
            ...
            console.log("compareWithPTree_cutoff: [Match!] returning true...")
            return true
        }
        ...
        compareWithPTree(root) {
            ...
            while (q.length) {
                ...
                if (first_round_flag) {
                    first_round_flag -= 1
                    console.log("skipping first round")
                }
                else {
                    // console.log("pr:", pr);
                    // console.log("pr.ptr:", pr.ptr);    
                    // console.log(`comparing nodes: pr.ptr`);                    
                    // console.log(pr.ptr, vertex['d']);
                    if (!this.checkComply(pr.ptr, vertex['d'])) {
                        console.log(`-> no match on ${pr.ptr} and ${vertex['d']}`)
                        return false
                    } 
                    else {
                        console.log(`-> match on ${pr.ptr} and ${vertex['d']}`)
                    }
                }
                ...
                for (let child of vertex['c']) {                
                    let new_pr = new PropertyRecord()
                    let success = new_pr.growFrom(pr, child['n'])
                    console.log("pr: ", new_pr);
                    console.log("child['n']: ", child['n']);
                    if (!success) {
                        if (first_round_flag < 0){
                            console.log("unable to growfrom: ", pr, first_round_flag);
                            return false
                        }
                        console.log("growFrom() not success!")
                        return false
                    } else {
                        q.push(child)
                        q2.push(new_pr)
        ...
        checkComply(property, _dict) {
            ...
            if (this.isArraySetMap(property)) {
                if (_dict['t'] == 3 && _dict['v'] == CryptoJS.MD5(JSON.stringify(property)).toString()) return true                
                else return false; 
            }
    ...
    class Libraries {
        ...
        addLib(libindex, detectLocationRecord, str="") {
        ...
        findLibs(blacklist, depth_limit=10, size_limit=10000) {
            let visited_vertex_num = 1
            let found_root_names = {} // this is used to avoid extra detection on already determined object
            let partial_path_match = (detected_partial, partial) => {
                if(partial.length < detected_partial.length){return false;}                    
                for(let i = 0; i < detected_partial.length; i++){
                    if(detected_partial[i] !== partial[i]){
                        return false;
                    }
                }
                return true;
            ...
            // property record is probably something that denotes a node for search
            const window_pr = new PropertyRecord()
            const q = [window_pr]
            while (q.length) {
                if (timeout) break

                // there's tons of library object to be reviewed here
                // q: [Object, Object, ...]                    
                // examine one library object at a time                
                let pr = q.shift()
                if (pr.hasCircle())
                    continue            
                // skip comparison if the prefix of current path list matches with that of previous decided object
                if(pr.path_list[0] && found_root_names[pr.path_list[0]] && partial_path_match(found_root_names[pr.path_list[0]], pr.path_list))
                {
                    // console.log("skipped comparison")
                    // console.log("pr.path_list: ", pr.path_list);
                    // console.log("found_root_names[pr.path_list[0]]: ", found_root_names[pr.path_list[0]]);
                    continue;
                }                
                ...
                for (let i in this.libInfoList) {
                    // libInfo reads feature in from libraries.json                    
                    let libInfo = this.libInfoList[i]
                    let lib_exist = false
                    let j = 0 // use to go through all feature sets
                    ...
                    while (true) {                        
                        ...
                        if ((!libInfo.hasOwnProperty(featureName))) {
                            // only compare if the library feature is property generated
                            break
                        }
                        
                        let properties = libInfo[featureName]
                        // console.log(`detecting ${libInfo['libname']} with properties: ${properties} on ${pr.path_list}`)
                        let feature_hold = true
                        // let OSS_debug = false
                        // if(pr?.['path_list']?.[0] === 'easyinvoice' && libInfo['libname'] === "easyinvoice"){
                        //     OSS_debug = true
                        // }
                        for (let property of properties) {
                            // go through the fingerprints, 
                            // a library is said to be detected if all properties in a feature is found
                            let _pr = new PropertyRecord()                            
                            _pr.copy(pr) // each property will be assigned with one dedicated property record
                            // console.log("looking for property", property);
                            // now it's looking into all the possible path to look for a specific property
                            // ex: to find 'caller', go through every possible path that could lead to 'caller'
                            // ['_', 'extend', 'caller'], ['_', 'extendOwn', 'caller'] ...

                            // for debug
                            // const target_arrde = ["_pdf", "_elementId", "_totalPages", "_renderedPdf"]
                            for (let v of property) {    
                                // if(OSS_debug){  
                                //     console.log(`looking for: '${v}'`, _pr);
                                // }
                                if (!_pr.grow(v)){ // one level test
                                    feature_hold = false
                                    // if(OSS_debug){
                                    //     console.log("feature not grow at ", v, _pr)
                                    // }
                                    break
                                }                                
                            }
                            // if (_pr.ptr === undefined) {
                            //     if(OSS_debug){                                    
                            //         console.log("feature not hold", _pr)
                            //     }
                            //     feature_hold = false
                            // }
                        ...
                        if (feature_hold) {
                            ...
                            // for(const [key, value] of Object.entries(libInfo)){
                            //     console.log(key, value);
                            // }
                            // console.log("done")
                            // console.log(this.libInfoList[i]['libname'], ": feature hold")
                            // console.log("pr when feature hold: ", pr);
                            lib_exist = true
                            found_root_names[pr.path_list[0]] = pr.path_list;
                            break
                        }
                    }
                    ...
                    if (lib_exist) {
                        if(DEBUG){
                            this.addLib(i, pr, this.libInfoList[i]['libname'])
                        }
                        else {
                            this.addLib(i, pr)
                        }
                ...
                if (pr.depth() < depth_limit) {
                    for (let child_v of children) {
                        ...
                        let new_pr = new PropertyRecord()
                        new_pr.growFrom(pr, child_v) // grow the object to have one more level of information  
                        q.push(new_pr)
        ...
        async checkVersion(baseurl) {
            ...
            console.log("libs list: ", this.libs);
            for (let lib of this.libs) {                
                ...
                if (this.VerDe[v_func] != undefined && typeof this.VerDe[v_func] == 'function') {
                    ...
                    try {
                        ...
                        console.error(`Error initializing version object with locationPtr: ${lib.locationPtr()}`, e);
                    }
                }
                ...
                if (!lib.version || lib.version.isUnknown()) {
                    ...
                    if (libInfo.hasOwnProperty('versionfile')) {
                        // temporarily stop looking for version file
                        ...
                        if (file_exist && response.status == 200) {
                            ...
                            console.log(`comparing versionfile: ${baseurl}/versions/${libInfo['versionfile']}`);
                            console.log("version_dict", version_dict)                       
                            try {
                                for (let [_, version_entry] of Object.entries(version_dict)) {
                                    if (version_entry['comparison_result'] === undefined) {
                                        console.log("Comparing with pTree")
                                        // version_entry['comparison_result'] = lib.compareWithPTree(version_entry['pTree'])
                                        version_entry['comparison_result'] = lib.compareWithPTree_cutoff(version_entry['pTree'])
                                    }
                                    if (version_entry['comparison_result'] == true) {
                                        console.log("comparison_result == true")
                                        ...
                                        for (let S_index of version_entry['Sm']) {
                                            ...
                                            if (S_entry['comparison_result'] === undefined) {
                                                S_entry['comparison_result'] = lib.compareWithPTree_cutoff(S_entry['pTree'])
                                            }
                                        ...
                                        console.log(`${baseurl}/versions/${libInfo['versionfile']}: comparison_result: false`)
                                    }
                                }    
                            } catch (error) {
                                console.log(`error in version detection of ${libInfo.libname}`, error)
                            }
                        }
                        else {
                ...
                console.log(`${libInfo['versionfile']} with version: ${JSON.stringify(lib.version)}`)
        ...
        convertToJson2() {
            ...
            // Sort based on libname
            json_output.sort((a, b) => { return a.libname.localeCompare(b.libname)})
            console.log("json_output", JSON.stringify(json_output))
            return json_output
        }
    }
    ...
    window.addEventListener("message", async function (event) {
        if (event.data.type == 'detect') {            
            ...
            // console.log("libInfoList: ", libInfoList);
            let L = new Libraries(libInfoList)                        
            L.prefindLibs()
            console.log('==== pre libs found ====')
            console.log(L)
    ...
    class VersionDetermine {
        ...
        test_bootstrap (root) {       
            libobj = root.bootstrap || ((root['Alert'] && root['Alert']['VERSION']) ? root : undefined)
            return new Version([(libobj)['Alert']['VERSION']]) 
        }
        ...
        test_google_maps(root) {
            libobj = root.google || ((root['maps'] && root['maps']['version']) ? root : undefined)
            return new Version([(libobj)['maps']['version']])
        }
        ...
        test_flot(root) {            
            libobj = (root.$) || root.jQuery || (root['plot'] ? root : undefined)
            if (libobj || libobj['plot']) plot = libobj['plot']
            if (libobj['plot'] && libobj['plot']['version']) 
        ...
        test_jquerymobile(root) {
            const jq = (root.jQuery || root.$ || (root.mobile ? root : undefined))
            if (jq.mobile) return new Version([jq.mobile.version])
            return new Version([])
        }

        test_jquerytools(root) {
            const jq = (root.jQuery || root.$ || (root.tools ? root : undefined))
            if (jq.tools) return new Version([jq.tools.version])
            return new Version([])
        }

        test_jquery(root) {
            const jq = (root.jQuery || root.$ || (root.fn ? root : undefined))
            if (jq.fn) return new Version([jq.fn.jquery])
        }   

        test_jqueryui(root) {
            const jq = (root.jQuery || root.$ || (root.ui ? root : undefined))
        ...
        test_mapboxjs (root) {
            libobj = root.L || ((root.box) ? root : undefined)
            return new Version([(libobj).mapbox.VERSION])
        }
        ...
        test_momenttimezone (root) {
            return new Version([(root.moment.tz || root.tz).version])
        }
        ...
        test_qooxdoo (root) {
            if ((root.qxWeb || root) && (root.qxWeb || root).$$qx && (root.qxWeb || root).$$qx.$$environment) {
                const version = (root.qxWeb || root).$$qx.$$environment["qx.version"]
                return new Version([version])
            }
            return new Version([])
        }
        ...
        test_riot (root) {
            libobj = root.$ || (root.riot ? (root) : undefined)
            if ((libobj) && (libobj).riot && typeof (libobj).riot == 'string')
                return new Version([(libobj).riot])
            if ((root.riot))
        ...
        test_fancybox (root) {
            libobj = root.$ || root.jQuery || (root.fancybox ? root : undefined)
            if ((libobj) && (libobj).fancybox)
                return new Version([(libobj).fancybox.version])
        }
        ...
        test_SoundJS (root) {
            libobj = root.createjs || (root.SoundJS ? root : undefined)
            return new Version([(libobj).SoundJS.version])
        }
        ...
        test_tweenjs (root) {
            libobj = root.createjs || (root.TweenJS ? root : undefined)
        ...
        test_vuechartjs (root) {
            libobj = root.VueChartJs || (root.VueCharts ? root : undefined)
            if ((libobj).VueCharts && (libobj).VueCharts.version)
                return new Version([(libobj).VueCharts.version])
            return new Version([])
        }
