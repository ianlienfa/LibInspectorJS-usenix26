...
(function (sandbox) {
     ...
     * the result object in jalangiinstrumentation.ast_info, so it will be available to analyses at the
...
//        esnstrument.closeIIDMapFile();
    ...
    if (require.main === module) { // main script
        ...
        parser.addArgument(['--inlineIID'], {
            help: "Inline IID to (beginLineNo, beginColNo, endLineNo, endColNo) in jalangiinstrumentation.iids in the instrumented file",
            action: 'storeTrue'
        });
        parser.addArgument(['--inlineSource'], {
            help: "Inline original source as string in jalangiinstrumentation.iids.code in the instrumented file",
            action: 'storeTrue'
        });
