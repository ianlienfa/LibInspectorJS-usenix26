...
/*global astUtil acorn esotope jalangiinstrumentation */

//var StatCollector = require('../utils/StatCollector');
if (typeof jalangiinstrumentation === 'undefined') {
    jalangiinstrumentation = {};
}
...
(function (sandbox) {
    ...
    function instrumentFile() {
        ...
        parser.addArgument(['--inlineIID'], {help: "Inline IID to (beginLineNo, beginColNo, endLineNo, endColNo) in jalangiinstrumentation.iids in the instrumented file", action: 'storeTrue'});
        parser.addArgument(['--inlineSource'], {help: "Inline original source as string in jalangiinstrumentation.iids.code in the instrumented file", action: 'storeTrue'});
        ...
        if (fileName.endsWith(".js")) {
            // deal with workers by creating a separate (throwaway?)
            // jalangi handle and dependencies
            var jalangiRoot = getJalangiRoot();
            var workerhelp = "if (typeof jalangiinstrumentation === 'undefined') {"+fs.readFileSync(path.join(jalangiRoot, "src/js/Constants.js"))+fs.readFileSync(path.join(jalangiRoot, "src/js/runtime/analysis.js"))+fs.readFileSync(path.join(jalangiRoot, "src/js/instrument/esnstrument.js"))+"}";
            ...
            fs.writeFileSync(instFileName, workerhelp + instCodeAndData.code, "utf8");
        } else {
                        ...
                        switch (node.tagName) {
                            case 'head':
                                var fragment = parse5.parseFragment(
                                    '<!-- esnstrument_cli.js:221 --><meta http-equiv="Content-Type" content="text/html; charset=utf-8">' +
...
// depends on jalangiinstrumentation.instrumentCode
