...
// places required functions on jalangiinstrumentation
...
function setupConfig(instHandler) {
    var conf = jalangiinstrumentation.Config;
 ...
 * object.  The instrumented code will store the result object in jalangiinstrumentation.ast_info, so it will be
...
function instrumentString(code, options) {
    ...
    var result = jalangiinstrumentation.instrumentCode(instCodeOptions);
    clearConfig();
    if (options.astHandler) {
        var info = options.astHandler(result.instAST);
        if (info) {
            result.code = jalangiinstrumentation.Constants.JALANGI_VAR + ".ast_info = " + JSON.stringify(info) + ";\n" + result.code;
        }
    }
    return result;
}
