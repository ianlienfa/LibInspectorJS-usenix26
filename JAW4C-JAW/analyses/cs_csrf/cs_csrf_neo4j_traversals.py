...
def build_and_analyze_hpg(seed_url):
	...
	for each_webpage in webapp_pages:
		...
		if query:
			...
			out = neo4jDatabaseUtilityModule.exec_fn_within_transaction(CSRFTraversalsModule.run_traversals, navigation_url, webpage, each_webpage)
			# the 'out' here is related to '#' in url when running auxiliary queries, not being used here
