...
(function (sandbox) {
     ...
     * only ignoring calls to jalangiinstrumentation.I()
     */
    ...
    function serialize(root) {
        ...
        function canMakeSymbolic(node) {
            if (node.callee.object) {
                ...
                // we can replace calls to jalangiinstrumentation functions with a SymbolicReference iff they have an IID as their first
                // argument.  'instrumentCode', 'getConcrete', and 'I' do not take an IID.
                // TODO are we missing other cases?
                if (callee.object.name === 'jalangiinstrumentation' && callee.property.name !== "instrumentCode" &&
        ...
        var visitorPost = {
            'CallExpression':function (node) {
                try {
                    if (node.callee.object && node.callee.object.name === 'jalangiinstrumentation' && (node.callee.property.name === 'Se' || node.callee.property.name === 'Fe')) {
...
// exports jalangiinstrumentation.astUtil
