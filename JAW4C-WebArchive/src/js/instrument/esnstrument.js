...
//    }
    ...
    /**
     * instruments entry of a script.  Adds the script entry (jalangiinstrumentation.Se) callback,
     * and the jalangiinstrumentation.N init callbacks for locals.
     *
     */
    ...
    function regExpToJSON() {
        ...
        var obj = {
            type: 'jalangiinstrumentation.AST.REGEXP',
            value: str,
            glb: glb,
    ...
    function JSONParseHandler(key, value) {
        var ret = value, flags = '';
        if (typeof value === 'object' && value && value.type === 'jalangiinstrumentation.AST.REGEXP') {
            if (value.glb)
                flags += 'g';
            if (value.ignoreCase)
                flags += 'i';
...
//                var ret = prependScriptBody(node, body);
    ...
    function hoistFunctionDeclaration(ast, hoisteredFunctions) {
        ...
        if (ast.body) {
            var newBody = [];
            if (ast.body.length > 0) { // do not hoister function declaration before jalangiinstrumentation.Fe or jalangiinstrumentation.Se
                if (ast.body[0].type === 'ExpressionStatement') {
                    if (ast.body[0].expression.type === 'CallExpression') {
                        if (ast.body[0].expression.callee.object &&
                            ast.body[0].expression.callee.object.name === 'jalangiinstrumentation'
    ...
    function instrumentCode(options) {
        ...
        if (!skip && typeof code === 'string' && code.indexOf(noInstr) < 0) {
            try {
                ...
                iidSourceInfo = {};
                var newAst;
                if (Config && Config.ENABLE_SAMPLING) {
...
// exports jalangiinstrumentation.instrumentCode
// exports jalangiinstrumentation.instrumentEvalCode
// depends on acorn
// depends on esotope
// depends on jalangiinstrumentation.Constants
// depends on jalangiinstrumentation.Config
// depends on jalangiinstrumentation.astUtil
