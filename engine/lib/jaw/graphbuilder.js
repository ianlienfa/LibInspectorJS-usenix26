...
function checkForAliasingNative(aliasPairs, functionMap) {
    return new Promise((resolve, reject) => {        
        const nativeExecutable = pathModule.join(__dirname, '../../lib/jaw/aliasing/aliasing');
...
async function checkFunctionMapForPartialAliasing(pairs) {
    ...
    false && console.log('functionMap before: ', Object.keys(functionMap))
    ...
    false && console.log('functionMap after: ', Object.keys(functionMap))
}
...
GraphBuilder.prototype.getInterProceduralModelNodesAndEdges = async function(semantic_types, options){
    ...
    async function processPageScopeTree(scopeTree, pageIndex) {
        ...
        if(options.ipcg){
            ...
            false && console.log("IPCG function map: ", functionMap)
        }
            ...
            walkes(mainAST, {
                ...
                CallExpression: function(node, recurse){
                    if(options.ipcg && node && node.type === 'CallExpression' && node.callee){
                        ...
                        // CASE 6: IIFE syntax                        
                        else if(IIFE_CALLEE_TYPES.has(node.callee.type)){
                            var function_definition_node = node.callee;
                            var args = {};
                            for(var arg_i=0; arg_i<node.arguments.length; arg_i++){
                                var argumentNode = node.arguments[arg_i];
                                var argCode = escodegen.generate(argumentNode);
